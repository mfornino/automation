...
collapse  piship (sum) vship (sum) pay (sum) vadd (sum) prodw, by(industry_ifr19 year) 
...
save "$masterpath/out/ThetaProdTsIfr.dta", replace

* Compute average shares of production-line employees in each sector before 1980
collapse (mean) vadd_1989 (mean) thetaProd = share_prodw_t if year<1980 , by(industry_ifr19)


save "$masterpath/out/ThetaProdIFR.dta", replace
